...
import BuildConfig.Untar.UntarConfig, BuildConfig.Common.CommonConfig
...
untar_package_logger = logging.getLogger(BuildConfig.Logger.LoggerConfig.APP_LOGGER_NAME + UNTAR_PACKAGE_LOGGER_NAME)
...
class UntarPackageThread(threading.Thread):
	...
	def __init__(self, group=None, target=None, name=None,
		...
		self.tar_package_type = args[2] if len(args) == 3 else BuildConfig.Untar.UntarConfig.DEFAULT_TAR_PACKAGE_TYPE
	...
	def run(self):
		...
		try:
			...
			if TAR_FILE_LOCATION.endswith(BuildConfig.Untar.UntarConfig.TAR_EXTENSION):
				try:
					...
					untar_package_logger.info('Checking Extract Base Directory Path: {' +
								BuildConfig.Untar.UntarConfig.TAR_BASE_EXTRACT_DIRECTORY + '}. Creating if doesn\'t exists...')
					...
					if not os.path.exists(BuildConfig.Untar.UntarConfig.TAR_BASE_EXTRACT_DIRECTORY):
						os.mkdir(BuildConfig.Untar.UntarConfig.TAR_BASE_EXTRACT_DIRECTORY)
						# Logging a comment
						untar_package_logger.info('Created Extract Base Directory Path: {' +
													BuildConfig.Untar.UntarConfig.TAR_BASE_EXTRACT_DIRECTORY + '}')

					# Contruct the Final Directory Path.
					TAR_FINAL_EXTRACT_DIRECTORY = BuildConfig.Untar.UntarConfig.TAR_BASE_EXTRACT_DIRECTORY + \
				...
				# Put application level logging below.
				# Logging a comment
				untar_package_logger.error('Extraction Operation Failed:: Not a {' + BuildConfig.Untar.UntarConfig.TAR_EXTENSION +
