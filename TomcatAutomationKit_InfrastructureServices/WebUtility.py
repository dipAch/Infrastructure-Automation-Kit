...
import BuildConfig.IO.IOConfig, BuildConfig.Common.CommonConfig
...
# wide Logger name.
WEBUTILITY_LOGGER_NAME = '.WebUtility'

# Get the Logger Instance for the module.
webutility_logger = logging.getLogger(BuildConfig.Logger.LoggerConfig.APP_LOGGER_NAME + WEBUTILITY_LOGGER_NAME)
...
def get_link(target_url, link_pattern):
	...
	try:
		...
		# Logging a comment
		webutility_logger.info('[Function: {' + _function_name + '}] Request sent to URI: {' + target_url + '}')

		# Store the Response [or Content] from the URI.
		content_body    = response_object.read().decode('UTF-8')

		# Logging a comment
		webutility_logger.info('[Function: {' + _function_name + '}] Response Received from URI: {' + target_url + '}')
	except (URLError, HTTPError, ContentTooShortError, UnicodeDecodeError, IOError, OSError) as webutility_get_link_error:
		# Put logging below.
		webutility_logger.error('[Function: {' + _function_name + '}] URI Request Failed: ' + str(webutility_get_link_error))
		raise
	else:
...
def download_tar_binary(url_tar_file_name, tar_request_object):
	...
	try:
		...
		# Logging a comment
		webutility_logger.info('[Function: {' + _function_name + '}] Checking Tar Download Base Directory Path: {' +
										BuildConfig.Common.CommonConfig.TAR_DOWNLOAD_BASE + '}. Creating if doesn\'t exists...')

		# Check if the `TAR` base downloads directory exists.
		if not os.path.exists(BuildConfig.Common.CommonConfig.TAR_DOWNLOAD_BASE):
			os.mkdir(BuildConfig.Common.CommonConfig.TAR_DOWNLOAD_BASE)
			# Logging a comment
			webutility_logger.info('[Function: {' + _function_name + '}] Created Tar Download Base Directory Path: {' +
											BuildConfig.Common.CommonConfig.TAR_DOWNLOAD_BASE + '}')

		tar_file_name     = os.path.basename(url_tar_file_name)
		tar_file_location = BuildConfig.Common.CommonConfig.TAR_DOWNLOAD_BASE + tar_file_name
		...
		with open(tar_file_location, 'wb') as tar_object:
			while True:
				...
				tar_chunk = binary_response.read(BuildConfig.IO.IOConfig.CHUNK)
				if not tar_chunk:
					break
		...
		# Logging a comment
		webutility_logger.info('[Function: {' + _function_name + '}] Tar successfully Downloaded to Location: {' +
									tar_file_location + '}')
		...
		# Put logging below.
		webutility_logger.error('[Function: {' + _function_name + '}] Tar Download Failed: ' +
