...
import BuildConfig.Common.CommonConfig, BuildConfig.Tomcat.TomcatConfig, BuildConfig.Java.JavaConfig
...
task_manager_logger = logging.getLogger(BuildConfig.Logger.LoggerConfig.APP_LOGGER_NAME + TASK_MANAGER_LOGGER_NAME)
...
class TaskManager(object):
	...
	def __init__(self):
		...
		self._allowed_retries = BuildConfig.Common.CommonConfig.TASK_RETRIES
			...
			try:
				...
				for component in list(thread_for.keys()):
					if not thread_for[component].download_complete:
						# Logging a comment
						task_manager_logger.critical('[Class: {' + str(self.__class__) + '}] Downloader Thread Failed for Component: {' +
														component + '}')
				...
				if build_for == BuildConfig.Tomcat.TomcatConfig.ENVIRONMENT['BUILD_TARGET']:
					# Return the `TAR` package names.
					# These would be used by the `UNTAR` module
					# to extract the files to the local / shared filesystem(s).
					return {BuildConfig.Java.JavaConfig.JAVA_TAR_COMPONENT_NAME:
													thread_for[BuildConfig.Java.JavaConfig.JAVA_COMPONENT_NAME].tar_file_name,
								BuildConfig.Tomcat.TomcatConfig.TOMCAT_TAR_COMPONENT_NAME:
													thread_for[BuildConfig.Tomcat.TomcatConfig.TOMCAT_COMPONENT_NAME].tar_file_name}
			...
			try:
				...
				for component in list(thread_for.keys()):
					if not thread_for[component].untar_complete:
						# Logging a comment
						task_manager_logger.critical('[Class: {' + str(self.__class__) + '}] Extraction Thread Failed for Component: {' +
															component + '}')
